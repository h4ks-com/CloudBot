...
def get_short(link):
    print(link)
    soup = Soup(cast(str, get(link)))
    text = ""
    parser_output = cast(Soup, soup.find("div", {"class": "mw-parser-output"}))
    for p in cast(list[Soup], parser_output.find("p", mode="all")):
        add = p.strip().strip()
...
def next(api):
    ...
    temp = cast(Soup, page.find("div", {"class": "mw-search-result-heading"}))
    title = temp.text
    description = cast(
        Soup,
        page.find("div", {"class": "mw-search-result-data"}, mode="first"),
    ).text
    link = f"https://{urllib.parse.urlparse(api).netloc}{cast(Soup, temp.find('a')).attrs['href']}"
...
def aw(text, bot, nick):
    ...
    html = Soup(cast(str, page))
    pages[API_ARCH] = cast(
        list[Soup],
        html.find("li", {"class": "mw-search-result"}, mode="all"),
    )
    return next(API_ARCH)
