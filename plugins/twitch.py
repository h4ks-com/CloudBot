...
multitwitch_re = re.compile(
    r"(.*:)//(www.multitwitch.tv|multitwitch.tv)/(.*)", re.I
)
...
def twitch_lookup(location, reply):
    ...
    if _type and _id:
        if _type == "b":  # I haven't found an API to retrieve broadcast info
            ...
            data = http.get_json(
                "https://api.twitch.tv/kraken/videos/" + _type + _id
            )
            title = data["title"]
            playing = data["game"]
        ...
        data = http.get_json(
            "https://api.twitch.tv/kraken/streams?channel=" + channel
        )
        if data["streams"]:
...
def twitch(text, reply):
    """<channel name> - Show whether the channel is live"""
    text = text.split("/")[-1]
    if test_name(text):
        location = text
    else:
