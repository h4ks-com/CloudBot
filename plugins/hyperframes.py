...
PREAMBLE = (
    ...
    "`transcript` to the words; they force-align to the real audio into word-synced karaoke "
    "captions (no TTS). Size the scenes to sum to the audio's length — call video_analyze_audio on "
    "the media_id to get it. For a SONG (sung vocals over music) set the transcript's audio_kind "
    "to 'sung' so the vocals are isolated before aligning, or the backing track throws the word "
    "timing off. Give each scene a `video` clip (a downloaded screenshot / YouTube "
    "segment / image): every media_id you downloaded MUST appear as a scene visual or it is simply "
    "not in the video, so list them across the scenes rather than authoring custom HTML that "
