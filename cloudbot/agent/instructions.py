...
AGENT_INSTRUCTIONS = (
    ...
    "Always read the FULL file before editing. read_github_file caps output at 12000 chars; if "
    "the response shows 'TRUNCATED chars X-Y of Z total', call it again with offset=Y to fetch "
    "the rest. Repeat until you've seen the whole file. NEVER call edit_github_file on a file "
    "you've only seen partially — your 'complete content' will be missing the lines you didn't "
    "read, and the commit will silently delete them. The shrink guard will reject most of these "
    "but not all. You cannot run or test code, so reason carefully about correctness. "
